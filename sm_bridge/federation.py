...
``create_sm_router``). ``pull_deltas`` fetches a peer's deltas since a cursor and
applies the upserts and deletes into a local :class:`~sm_bridge.DeltaStore`.
``FederationPoller`` runs it on a background thread.
